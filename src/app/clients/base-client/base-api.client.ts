...
export abstract class BaseApiClient extends BaseAjaxClient {
        ...
        try {
            ...
            if (responseEntity != null)
                ...
                if (additionalRequestDetails.enableAuth === Authentication.true) {
                    let token: string = await this.storageservice.getDataFromAnyStorage(AppConstants.DATABASE_KEYS.ACCESS_TOKEN);
                    if (token == null || token === '')
    ...
    private async AddCommonHeaders(commonHeaders: IDictionaryCollection<string, string> | null): Promise<IDictionaryCollection<string, string>> {
        ...
        commonHeaders.Add(AppConstants.HEADER_NAMES.ApiType, AppConstants.HEADER_VALUES.ApiType);
        commonHeaders.Add(AppConstants.HEADER_NAMES.DevApk, AppConstants.HEADER_VALUES.DevApk);
        commonHeaders.Add(AppConstants.HEADER_NAMES.AppVersion, AppConstants.HEADER_VALUES.AppVersion);
        // //cors headers
