...
export class AuthGuard {
  ...
  async canActivate(
    ...
    if (isValidRole)
      ...
      this.router.navigate([AppConstants.WEB_ROUTES.ENDUSER.HOME]);
      return false;
    }
  }
  ...
  async canActivateChild(
    ...
    state: RouterStateSnapshot): Promise<boolean> {
    const token: string = await this.storageService.getFromStorage(AppConstants.DATABASE_KEYS.ACCESS_TOKEN);
    return !jwtHelper.isTokenExpired(token);
  }

  async IsTokenValid(): Promise<boolean> {
    const token: string = await this.storageService.getDataFromAnyStorage(AppConstants.DATABASE_KEYS.ACCESS_TOKEN);
    return token != undefined && token != null && token != "" && !jwtHelper.isTokenExpired(token);
  }
